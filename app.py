...
from langchain.chains.question_answering import load_qa_chain #for chat
...
def get_conversational_chain():
    ...
    make sure to provide all the possible details, 
    and if the answer is not present in the provided context, 
    just say "Answer is not available in the provided context", 
...
"""
    ...
    model=ChatGoogleGenerativeAI(model='gemini-pro', tempreture=1)
...
def main():
    ...
    with st.sidebar:
        st.title('Menu:')
        pdf_docs=st.file_uploader('Upload your PDF Files and Click Process', type='pdf', accept_multiple_files=True)
